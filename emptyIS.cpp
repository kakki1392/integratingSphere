...
int main(){
	...
	for(int j=0; j<N_R; j++){
		...
		for(int k=0; k<N_avg; k++){
			...
			initPhotons(photons, R_vec[j], z_s, cos_theta0, a_s, b_s, G);
	...
	ofstream myFile;
	myFile.open("data/emptyIS/air_9.dat");
	myFile << "#Empty integrating sphere. Lambertian reflectance. No ports for fluid." << endl
		...
		<< "#Seed: " << seed << endl << "#" << endl
		<< "#R <eps_c> <eps_e> <eps_w> stddev_eps_c stddev_eps_e stddev_eps_w" << endl;
	for(int i=0; i<N_R; i++){
		myFile << setprecision(5) << fixed << R_vec[i] << " " << setprecision(10) << fixed << eps_c_mean[i] << " " << eps_e_mean[i] << " " << eps_w_mean[i]
		       	<< " " << eps_c_std[i] << " " << eps_e_std[i] << " " << eps_w_std[i] << endl;
	}
	myFile.close();
