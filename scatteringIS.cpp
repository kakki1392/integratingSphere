...
int main(){
	...
	vector<double> albedo_vec(N_alpha_s);
	vector<double> alpha_t_vec(N_alpha_s);

	vector<double> eps_c_mean(N_alpha_s);
	vector<double> eps_e_mean(N_alpha_s);
	vector<double> eps_w_mean(N_alpha_s);
	vector<double> eps_i_mean(N_alpha_s);

	vector<double> eps_c_std(N_alpha_s);
	vector<double> eps_e_std(N_alpha_s);
	vector<double> eps_w_std(N_alpha_s);
	vector<double> eps_i_std(N_alpha_s);

	vector<double> mean_path_avg(N_alpha_s);
	vector<double> std_mean_path_avg(N_alpha_s);
	vector<double> std_path_avg(N_alpha_s);
	vector<double> std_std_path_avg(N_alpha_s);

	alpha_s_vec[0] = alpha_s0;
	alpha_t_vec[0] = alpha_a + alpha_s_vec[0];
	albedo_vec[0] = alpha_a/(alpha_s_vec[0] + alpha_a);

	for(int i=1; i<N_alpha_s; i++){
	...
	*/
	...
	myFile << "#Empty integrating sphere. Lambertian reflectance. No ports for fluid." << endl
		...
		<< "#Number of alpha_s-values: N_alpha_s = " << N_alpha_s << endl
		<< "#Delta alpha_s: d_alpha_s = " << d_alpha_s << endl
		<< "#Seed: " << seed << endl << "#" << endl
		<< "#alpha_s <eps_c> <eps_e> <eps_w> <eps_i> std_eps_c std_eps_e std_eps_w std_eps_i <<path>> std<path> <std_path> <<std_path>>" << endl;
	for(int i=0; i<N_alpha_s; i++){
		myFile << setprecision(5) << fixed << alpha_s_vec[i] << " " << setprecision(10) << fixed << eps_c_mean[i] << " " << eps_e_mean[i] << " " << eps_w_mean[i]
	...
	vector<double> A(N_alpha_s);
	vector<double> T(N_alpha_s);
	vector<double> L_eff(N_alpha_s);
	for(size_t i=0; i<N_alpha_s; i++){
		T[i] = eps_c_mean[i]/eps_c_ref_mean;
		A[i] = std::log(1/T[i]);
		L_eff[i] = A[i]/alpha_s_vec[i];
	}
	...
	myFile2 << "#Empty integrating sphere. Lambertian reflectance. No ports for fluid." << endl
		...
		<< "#Number of photons: N = " << N_photons << endl
		<< "#Number of averages: N_avg = " << N_avg << endl
		<< "#Number of alpha-values: N_alpha_s = " << N_alpha_s << endl
		<< "#Delta alpha_s: d_alpha_s = " << d_alpha_s << endl
		<< "#Seed: " << seed << endl << "#" << endl
		<< "#alpha T A L_eff" << endl;
	for(int i=0; i<N_alpha_s; i++){
		myFile2 << setprecision(5) << fixed << alpha_s_vec[i] << " " << setprecision(10) << fixed << T[i] << " " << A[i] << " " << L_eff[i] << endl; 
	}
	myFile2.close();
